...
const handleDownloadPrescription = () => {
  ...
  // Website name (GREEN + BOLD)
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(19); // slightly stronger
  pdf.setTextColor(22, 163, 74); // website green (#16a34a)
  pdf.text("KisanSetu – Disease Prescription", centerX, y, { align: "center" });
  ...
  pdf.setFontSize(11);
  pdf.text(`Date: ${prescription.date}`, 20, y); y += 7;
  pdf.text(`Disease Detected: ${prescription.disease}`, 20, y); y += 7;
  pdf.text(`Severity: ${prescription.severity}`, 20, y); y += 7;
  pdf.text(`Weather Conditions: ${prescription.weather}`, 20, y); y += 7;
  pdf.text(`Land Area: ${landAcres} acres`, 20, y); y += 12;

  // Section header (BOLDER)
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(15); // increased for emphasis
  pdf.text("Recommended Treatments", 20, y);
  y += 12;
  ...
  prescription.treatments.forEach((t, index) => {
    // Treatment name (BOLDER)
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(12);
    pdf.text(`${index + 1}. ${t.name}`, 20, y);
    y += 6;

    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(11);
    pdf.text(
      `Quantity: ${t.quantity.toLocaleString("en-IN", { maximumFractionDigits: 2 })} ${t.unit}`,
      25,
      y
    );
    y += 6;
    ...
    pdf.text(
      `Cost: Rs. ${t.totalCost.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`,
      25,
      y
    );
    y += 10;
